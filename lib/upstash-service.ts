import type { Transaction, RiskStatus, Settings } from './types';
...
try {
  ...
  if (redisUrl && redisToken) {
    ...
    throw new Error('Upstash Redis 环境变量未配置');
  }
} catch (error) {
  console.log('⚠️  使用 Mock Redis 服务（开发环境）');
...
export class UpstashService {
  ...
  static async deleteTransaction(id: string): Promise<void> {
    try {
      ...
      await redis.set('transactions', filteredTransactions);
    } catch (error) {
  ...
  static async getRiskStatus(): Promise<RiskStatus> {
    try {
      const riskStatus = await redis.get('riskStatus');
      return riskStatus || {
        isInRisk: false,
        consecutiveLosses: 0,
      };
  ...
  static async updateRiskStatus(status: RiskStatus): Promise<void> {
    try {
      await redis.set('riskStatus', status);
    } catch (error) {
  ...
  static async updateSettings(updates: Partial<Settings>): Promise<Settings> {
    try {
      ...
      await redis.set('settings', newSettings);
      return newSettings;
    } catch (error) {
  ...
  static async resetAllData(): Promise<void> {
    try {
      // 清空所有交易记录
      await redis.set('transactions', []);
      
      // 重置风险状态
      await redis.set('riskStatus', {
        isInRisk: false,
        consecutiveLosses: 0,
      });
      
      // 重置设置为默认值
      await redis.set('settings', {
        currency: '¥',
        riskAlertEnabled: true,
        theme: 'light',
        autoBackup: true,
  ...
  static async getSystemOverview(): Promise<{
    ...
    try {
      ...
      const lastResetTime = await redis.get('lastResetTime');
  ...
  static async recordResetTime(): Promise<void> {
    try {
      await redis.set('lastResetTime', new Date().toISOString());
    } catch (error) {
